document.addEventListener("DOMContentLoaded", function() {
	...
	$(document).on('click', '.edit-reply-button', function() {
		...
		$.ajax({
			type: 'PUT',
			url: '/comments/updateReply/' + commentNo,
			beforeSend: function(xhr) {
				xhr.setRequestHeader(header, token);
				xhr.setRequestHeader("Content-Type", "application/json");
			},
			...
			success: function(response) {
				if (response === "권한 확인됨") {
					var newCommentText = prompt("대댓글을 수정하세요:", '');
					if (newCommentText !== null && newCommentText.trim() !== '') {
						$.ajax({
							type: 'PUT',
							url: '/comments/updateReply/' + commentNo,
			...
			success: function(comments) {
				...
				if (comments.length > 0) {
					$.each(comments, function(index, comment) {
						if (!comment.parentComment) { // 부모 댓글이 없는 경우에만 댓글로 처리
							var commentHtml = `
                                <div class="comment" id="comment-${comment.commentNo}">
                                    ...
                                    <div id="reply-list-${comment.commentNo}" class="reply-list">
                                        <!-- 대댓글 목록은 loadReplies 함수에서 별도로 로드 -->
                                    </div>
                                </div>
                            `;
    ...
    console.log("Loading replies for parentCommentId: " + parentCommentId);
    $.ajax({
        type: 'GET',
        url: '/comments/replies/' + parentCommentId,
        success: function(replies) {
            var replySection = $('#reply-list-' + parentCommentId);
            replySection.empty();

            if (replies.length > 0) {
                $.each(replies, function(index, reply) {
                    console.log(reply); // 전체 reply 객체 출력
                    if (reply.communityBoard && reply.communityBoard.mateno) {
                        var replyHtml = `
                            <div class="reply">
                                <p>${reply.commentWriter}: ${reply.commentText}</p>
                                <button class="edit-reply-button" data-commentid="${reply.commentNo}" data-mateno="${reply.communityBoard.mateno}">수정</button>
                            </div>
                        `;
                        replySection.append(replyHtml);
                    } else {
                        console.error('reply.communityBoard or reply.communityBoard.mateno is null or undefined');
                    }
                });
            }
        },
        error: function(xhr, status, error) {
            console.error('대댓글 조회 중 오류가 발생했습니다:', xhr.responseText);
        }
    });
}
});
